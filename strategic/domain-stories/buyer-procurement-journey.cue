...
buyerProcurementJourney: artifact_schemas.#DomainStory & {
	...
	steps: [{
		...
		workItem: {
			...
			boundedContextRef: "ssc"
			commandRefs: ["cmd-propose-counter-terms"]
		...
		rationale: "O passo que as fontes chamam de 'arte' e apontam como o que salva o fluxo de caixa. No exame original (2026-07-12) era o vazio mais denso em valor da story — zero elementos em qualquer BC; FECHADO em 2026-07-28 pelo WI-161: rodadas de contraproposta→revisão|recusa intra-open (3 commands + 3 events internal, molde dos fatos de cotação — confidencialidade preservada), condições de pagamento e entregas programadas ESTRUTURADAS (vo-payment-terms/vo-delivery-schedule), e a regra de ouro inv-negotiated-terms-materialize-on-quotation: só a revisão do fornecedor materializa condições na cotação — o gate de procedência do adr-177 resolve o preço FINAL na cotação vencedora por construção ('as condições finais que a decisão formaliza'). A mesa da negociação é o próprio mapa (prj/qry-quotation-map, agora com as rodadas), refs deste passo. Revisão de 2026-09-03: o alvo de contraproposta é preparado pelo agente (act-prepare-counter-proposal, propose-and-wait) e EMITIDO pelo comprador — quem invoca cmd-propose-counter-terms segue sendo sh-08. Revisão de 2026-09-07: os refs MUDARAM. A negociação é BILATERAL por construção, e este passo narra um lado só. cmd-revise-quotation e cmd-decline-counter-terms são atos do FORNECEDOR — o domain-model os declara literalmente ('Fornecedor revisa a própria cotação… supplierRef deve match'; 'Fornecedor declina a contraproposta e mantém as condições vigentes'), e a inv-negotiated-terms-materialize-on-quotation faz disso lei estrutural: 'a cotação é do fornecedor; o comprador nunca escreve nela… nenhum caminho de escrita do comprador alcança a ent-quotation'. Mantê-los aqui atribuía a sh-08 comandos que o modelo IMPEDE sh-08 de invocar. Saíram para a ds-supplier-network-journey, passos 7 e 8, escritos no MESMO commit — refs não se movem em dois tempos: tirar de um lado e pôr no outro é uma operação só, mesmo quando o trabalho que a produziu teve etapas. Os eventRefs PERMANECEM os três, e a distinção é deliberada: aqui eles são fatos que o passo CONSOME, não produz. O comprador lê no mapa o desfecho da rodada — a fila dele 'distingue manteve de aguardando' (rationale do cmd-decline-counter-terms) —, e tirar os eventos tiraria o que ele lê. O schema #StoryStep tem UM actorRef e nenhum campo de contraparte, então a bilateralidade não cabe dentro do passo: ela vive na aresta entre as duas stories, declarada aqui e no espelho."
	}, {
		actorRef: "sh-08"
		...
		workItem: {
			...
			eventRefs: ["evt-purchase-order-emitted", "evt-purchase-requisition-converted"]
			policyRefs: ["pol-purchase-order-converts-requisition"]
			readModelRefs: ["prj-purchase-orders"]
			termRefs: ["term-purchase-order", "term-sourcing-authority"]
		}
		rationale: "O desfecho do recorte, com lar completo (emissão sob authority validada) — e a fronteira da dobra cunha→core: evt-purchase-order-emitted é o que o cmt consome para iniciar o compromisso, fora deste recorte. Revisão de 2026-09-07 (fatia 0): o fecho do ciclo entrou — pol-purchase-order-converts-requisition dispara em evt-purchase-order-emitted, emite cmd-convert-requisition e a requisição vai de approved a converted (evt-purchase-requisition-converted). Achado por derivação (policy do BC disparada por evento que o passo declara), e é o PRIMEIRO policyRefs preenchido em qualquer story: o sc-ds-06 deixa de ser vacuamente verde."
	}]
