...
def043: artifact_schemas.#DeferredDecision & {
	id:     "def-043"
	title:  "Vendor-of-record do WorkflowPort deferido até o golden-example (decisão JIT)"
	date:   "2026-06-28"
	status: "open"

	description: """
		adr-141 fixa o contrato do WorkflowPort — o Port da SoT da execução (categoria: workflow /
		durable-execution engine): operações, PortResult<T>, value classes na fronteira, obrigações
		de contract-test. Fica deferida a seleção do vendor de durable execution que IMPLEMENTA esse
		Port como adapter em platform/adapters/ — categoria de runtime atrás do Port (P2), não
		contrato de domínio.
		"""
	...
	triggerCalibrationRationale: """
		...
		persistente (não-in-memory) do WorkflowPort a entrar em construção — só fica devida em evento
		de runtime que vive no mesh-runtime (convenção platform/adapters/workflow-<vendor>/) e NÃO tem
	...
	triggers: [{
		kind:   "manual-review"
		reason: "1º adapter persistente (não-in-memory) do WorkflowPort a entrar em construção. A convenção platform/adapters/workflow-<vendor>/ vive no mesh-runtime e o vendor É a decisão — sem file-exists honesto a partir do mesh-spec."
	}, {
		kind:       "temporal"
		maxAgeDays: 180
	}]
