...
def042: artifact_schemas.#DeferredDecision & {
	...
	triggerCalibrationRationale: """
		...
		persistente (não-in-memory) do LedgerPort a entrar em construção — só fica devida em evento de
		runtime que vive no mesh-runtime (convenção platform/adapters/ledger-<vendor>/) e NÃO tem
	...
	triggers: [{
		kind:   "manual-review"
		reason: "1º adapter persistente (não-in-memory) do LedgerPort a entrar em construção. A convenção platform/adapters/ledger-<vendor>/ vive no mesh-runtime e o vendor É a decisão deferida — sem file-exists honesto a partir do mesh-spec."
	}, {
		kind:       "temporal"
		maxAgeDays: 180
	}]
