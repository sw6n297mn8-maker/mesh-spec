...
agentGovernanceGuide: artifact_schemas.#ProductionGuide & {
	...
	sections: {
		"routing-and-blast-radius": {
			...
			heuristics: [
				...
				"Bounded wait queue governance (per adr-075 Caminho D'): rotas com channel='alert-and-block' que implementam bounded wait + escape hatch supervisionado DEVEM declarar maxQueueDepth + maxQueueAge + overflowPolicy fail-safe-only para evitar backlog invisível. #OverflowPolicy.action é restricted a 'auto-cancel-and-escalate' Phase 0 (NÃO auto-approve sob queue pressure — fail-permissive sob pressure é classe de erro adversarial; DDoS-induced overflow induziria invariant bypass se permitido). cancelReasonCode MUST match um dos reasonCodes em vo-cancellation-reason (OR equivalente) do domain-model do BC alvo (CUE não resolve cross-context enum; runner cross-file futuro valida). escalateVia re-routes overflow event via existing escalation taxonomy.",
			]
			doneCriteria: "agentRef + governanceGlobalVersion + lifecycleStage declarados; cada category em agent-spec.escalationConditions[] tem escalationRouting correspondente OR rationale explícito de fallback global; routes alert-and-block declaram block scope explícito per tq-gvg-10; routes com bounded wait declaram queue governance fields (maxQueueDepth + maxQueueAge + overflowPolicy fail-safe-only) per adr-075; blastRadiusCaps proporcionais à criticidade BC + sanity check daily ≥ concurrent + monotonicidade vs lifecycleStage (tq-gvg-07); precedência declarada quando categories podem coexistir (tq-gvg-05); envelope contém apenas control plane (tq-gvg-09); founder aprovou routing matrix antes de proceder à section 2."
		...
		"drift-and-calibration": {
			...
			heuristics: [
				...
				"Signal-as-contract pattern (per adr-075 Caminho D'): regression triggers que precisam scope dinâmico (actor-localized containment, category-affected systemic, etc.) declaram scopedBySignal: #ObservabilitySignalRef apontando para signal code do agent-spec. Scope resolution deterministic via signal payload snapshot at trigger time — runtime evaluation derives state from audit log; envelope declares contracts, not state. Audit trail captures {trigger code, signal-payload-snapshot, action, resolved-scope} para auditability completa. Signal referenciado DEVE declarar payloadFields em agent-spec.observability.signals[i] com fields que carry scope info (e.g., requesterRef, categoryRef). Substitui runner-implicit actor-scoping (anti-pattern: 'runner MUST implement') por contract explícito.",
				"Clearance condition pattern (per adr-075 Caminho D'): regression triggers para sustained containment effects (Tier 2 Hard, Tier 3 Systemic em Actor-Scoped Adaptive Containment) declaram clearanceCondition: #ClearanceCondition discriminated union. Phase 0 single variant #ClearanceByNoSignalInWindow (signalRef + window + scope enum + maxOccurrences int — todos campos typed, ZERO prose). Variants futuros (clearance-by-metric-below-threshold, by-duration-elapsed, by-manual-review) adicionados como extensions não-breaking quando primeira instância requisitar (def-013 trigger 2). Estado deriva de audit log (single source of truth); pattern paralelo a circuit breaker / rate limiter em sistemas críticos.",
			]
			doneCriteria: "driftDetection.cadence + métricas mensuráveis (baseline + threshold) declaradas; calibration.promotionCriteria com volume + period + approval-rate; regressionTriggers com immediateAction enum + tolerance-zero para boundary violations; regression triggers actor-scoped declaram scopedBySignal apontando para signal code valido em agent-spec (per adr-075); regression triggers para sustained containment declaram clearanceCondition discriminated union (per adr-075); calibration reconstrutível verificada; ≥1 automatic enforcement binding drift→action declarado quando aplicável (tq-gvg-06); failureHandling field preenchido per #FailureHandling shape cobrindo onAgentError/onTimeout/onRepeatedFailure (tq-gvg-08, schema first-class per adr-058); founder aprovou thresholds antes de section 3."
	...
	finalValidation: {
		steps: [
			...
			"Verificar signal-as-contract refs (per adr-075 Caminho D'): cada #RegressionTrigger.scopedBySignal? em envelope aponta para signal code valido em agent-spec.observability.signals[].code. Signal referenciado declara payloadFields contendo fields de scope (e.g., requesterRef, categoryRef) per pattern signal-as-contract. Audit trail captura snapshot do payload at trigger time — runtime evaluation derives state from audit log; envelope declares contracts, not state. Refs quebrados (signal inexistente OR payloadFields vazios) são gap silencioso na auditability.",
			"Verificar clearance condition typed (per adr-075 Caminho D'): cada #RegressionTrigger.clearanceCondition? em envelope usa #ClearanceCondition discriminated union (Phase 0: variant #ClearanceByNoSignalInWindow apenas). Variant declara signalRef #ObservabilitySignalRef + window #DurationDescriptor + scope enum (inherit-from-trigger | global) + maxOccurrences int — todos campos typed/enums/refs/numerics, ZERO prose. Runtime evaluation deriva state from audit log via query: 'count of signal occurrences matching scope dentro de window ≤ maxOccurrences? → clearance fires'.",
			"Verificar queue governance fail-safe (per adr-075 Caminho D'): rotas com channel='alert-and-block' que implementam bounded wait declaram maxQueueDepth + maxQueueAge + overflowPolicy. #OverflowPolicy.action é restricted a 'auto-cancel-and-escalate' Phase 0 (fail-safe-only — auto-approve sob queue pressure é classe de erro adversarial vetada por design). cancelReasonCode MUST match um dos reasonCodes em vo-cancellation-reason.reasonCode constraint do domain-model do BC alvo (CUE não resolve cross-context enum; runner cross-file futuro valida). escalateVia re-routes overflow event via existing escalation taxonomy do envelope.",
