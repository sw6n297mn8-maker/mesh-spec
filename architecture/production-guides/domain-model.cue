...
//
// Iteração pós-SSC (sessão 2026-05-05, post-commit SSC 7da909a):
// 4 disciplinas adicionadas a partir de erros revelados durante
// authoring de SSC red-team — aggregate identity must exist before
// outcome (rootIdentity de outcome opcional como sourcingDecisionId
// quebra fluxos cancelados), entity vs value-object com lifecycle
// mutável (Quotation como VO mesmo tendo identidade + submitted →
...
domainModelGuide: artifact_schemas.#ProductionGuide & {
	...
	_qualityCriteria: {
		criteria: [{
			...
			id:          "tq-dmg-10"
			description: "Guide enforça rootIdentity existente desde a criação do aggregate"
			...
			id:          "tq-dmg-12"
			description: "Guide enforça coevolução do agent-spec do BC ao alterar building blocks operáveis"
			test:        "FinalValidation inclui passo explícito de verificar, ao criar/alterar/remover building block das 6 famílias operáveis (aggregates, commands, events, invariants, projections, domainServices), se o(s) agent-spec(s) do BC precisam coevoluir (operationalScope/actions/scopeExclusions) E se a prosa das actions do agente continua verdadeira contra o modelo alterado — coevoluindo no mesmo ciclo ou justificando a não-edição explicitamente. Verificado por inspeção do guide."
			severity:    "warn"
			rationale:   "WI-151/152/153 materializaram building blocks sem coevoluir os agent-specs de p2p/ssc/bdg — drift silencioso que o gate mecânico (sc-ag-02, adr-175) só pega na classe-1 (id nem coberto nem excluído). A classe-2 (prosa do agente envelhecida — action descrevendo emissão que o re-papel moveu) é interpretativa: P10 veda gate LLM, então vive como disciplina de processo no guide + advisory. Warn e não fail: a decisão de coevolução pertence ao founder; o critério garante que o guide força a PERGUNTA, não a resposta."
		}]
		rationale: "12 critérios cobrem disciplinas core para autoria de domain-model: integridade referencial catalog↔aggregates (tq-dmg-01), behavior-first ordering (tq-dmg-02), lifecycle válido (tq-dmg-03), glossary alignment (tq-dmg-04), lifecycle reachability (tq-dmg-05), outcome split em commands (tq-dmg-06), stateless aggregate justification (tq-dmg-07), fields-states consistency (tq-dmg-08), cross-aggregate state dependency declaration (tq-dmg-09), aggregate identity must exist before outcome (tq-dmg-10), canvas semantic decision propagation (tq-dmg-11), agent-spec coevolution (tq-dmg-12). Scope é disciplinas que protocol enforce via process; cobertura completa dos 17+ tq-dm-XX do schema vive em finalValidation.steps. Critérios 05-08 derivam de gaps revelados durante authoring de IDC (commit 14063de). Critério 09 deriva de adr-055 (cross-aggregate state dependency first-class). Critério 10 deriva de SSC red-team (commit 7da909a). Critério 11 promovido de finalValidation step para criterion após review do PG (mesma sessão SSC bootstrap). Critério 12 deriva do drift agente↔modelo das fatias WI-151/152/153 (adr-175 — elo duplo: critério assegura que o guide contém o passo; o passo em finalValidation é o operativo)."
	}
	...
	sections: {
		"context-and-behavior-first-catalog": {
			...
			heuristics: [
				...
				"Command lifecycle separation: se processo tem lifecycle público (open/concluded/cancelled, pending/approved/rejected), separar commands em 3 fases — criação/abertura (cmd-open-X), mutações intra-state (cmd-submit-Y, cmd-update-Z), conclusão/terminação (cmd-conclude-X, cmd-cancel-X). NÃO esconder criação do aggregate dentro de command de outcome final (ex.: cmd-make-decision não deve também abrir RFQ); NÃO fundir abertura + recepção + conclusão em command monolítico. Exceção: operação atômica one-step (sem janela observável de intermediate state) pode fundir, com rationale explícito justificando ausência de lifecycle público.",
			]
		...
		"aggregates-and-wiring": {
			...
			heuristics: [
				...
				"Aggregate identity must exist before outcome (tq-dmg-10): se aggregate pode existir antes de outcome final (processo cancelável antes de decisão; pedido rejeitável antes de aprovação; verificação abandonável antes de conclusão), rootIdentity é identity do processo inicial (rfqId, requestId, verificationId), NÃO identity de outcome opcional (sourcingDecisionId, approvalId, verificationResultId). Outcome ID vira field opcional do aggregate (populated apenas no state terminal de sucesso) e/ou field de event terminal. Teste: o aggregate consegue ser representado consistentemente em estado cancelado/rejeitado sem outcome? Se não, rootIdentity está errado.",
				"Entity com lifecycle vs value-object: se conceito tem identity própria persistente, é referenciado por command posterior (withdraw, cancel, update), ou muda status sem perder identidade (submitted → withdrawn; pending → confirmed), modelar como entity nested, NÃO value-object. Value-object é imutável por definição — não deve carregar lifecycle mutável nem field 'status' que evolui. Se você se vê propondo um VO com field status que muda, promova a entity nested no aggregate apropriado.",
			]
	...
	finalValidation: {
		steps: [
			...
			"Verificar aggregate identity precedência (tq-dmg-10): para cada aggregate, perguntar 'pode existir antes de outcome final (cancelado/rejeitado/abandonado sem outcome)?'. Se sim, rootIdentity é identity do processo inicial; outcome IDs são fields opcionais. Se rootIdentity é identity de outcome (XxxResultId, XxxDecisionId), reclassificar: ou outcome é obrigatório por construção (sem branch de cancelamento), ou rootIdentity está errado.",
			...
			"Verificar coevolução do agent-spec (tq-dmg-12): ao criar/alterar/remover building block das 6 famílias operáveis (aggregates, commands, events, invariants, projections, domainServices), verificar se o(s) agent-spec(s) do BC (contexts/{bc}/agents/) precisam coevoluir — operationalScope/actions para cobertura, scopeExclusions para exclusão consciente (critério de legitimidade: adr-175) — E se a PROSA das actions/descriptions do agente continua verdadeira contra o modelo alterado (ex.: action que diz 'emite EventoX' quando o re-papel moveu a emissão). Coevoluir no mesmo ciclo OU justificar a não-edição explicitamente na proposta ao founder. A camada mecânica (id nem coberto nem excluído) é do sc-ag-02; este passo cobre a decisão de coevolução e a classe-2 (prosa envelhecida) que gate nenhum vê.",
