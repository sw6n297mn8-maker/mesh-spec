...
adrGuide: artifact_schemas.#ProductionGuide & {
	...
	_qualityCriteria: {
		criteria: [{
			...
			id:          "tq-adrg-03"
			description: "Guide produz traceability metadata com paths reais nos 3 fields schema-supported (per adr-059)"
			test:        "Process da consequences-rationale-and-traceability exige declaração de affectedArtifacts (paths existentes alterados, required) + plannedOutputs (paths novos criados pela decisão como output direto, optional per adr-059) + derivedArtifacts (regenerados/derivados como consequência indireta, optional). Disciplina 3-way schema-supported: existing-altered → affectedArtifacts; new-created → plannedOutputs; derived-regenerated → derivedArtifacts. ADRs pré-adr-059 grandfathered (sem plannedOutputs; affectedArtifacts conflate existing-altered + new-created). gapPolicy proíbe paths especulativos. finalValidation verifica tq-adr-03."
			severity:    "fail"
			rationale:   "tq-adr-03 do schema já é fail; guide reitera. Per adr-059, schema separa as 3 categorias em fields first-class — disciplina 3-way agora declarativa, não conceitual narrative. ADRs pós-adr-059 SHOULD usar plannedOutputs para new-created; ADRs antigas grandfathered sem migration retroativa (Parts 2/3 do C3 plan deferidas). Schema field optional preserva backward compat sem comprometer adoption forward-looking."
		}, {
			id:          "tq-adrg-04"
	...
	prerequisites: {
		...
		collectFromFounder: [
			...
			"Traceability: paths sob disciplina 3-way schema-supported (per adr-059) — existing-altered → affectedArtifacts (required); new-created → plannedOutputs (optional, novo per adr-059); derived-regenerated → derivedArtifacts (optional). ADRs pré-adr-059 grandfathered (sem plannedOutputs; affectedArtifacts conflate existing-altered + new-created). Agente NÃO inclui paths especulativos; tq-adr-03 fail",
	...
	sections: {
		...
		"consequences-rationale-and-traceability": {
			target:    "#ADR"
			objective: "Compor closing analysis — consequences positivas e negativas, rationale synthesis, e traceability metadata (affectedArtifacts, plannedOutputs, derivedArtifacts, principlesApplied) com paths reais sob disciplina 3-way schema-supported (per adr-059)."
			process: [{
				...
				action: "Declarar affectedArtifacts, plannedOutputs e derivedArtifacts sob disciplina 3-way schema-supported"
				detail: "Disciplina 3-way schema-supported per adr-059: existing-altered (arquivos existentes alterados pela decisão) → affectedArtifacts (required); new-created (arquivos novos criados pela decisão como output direto) → plannedOutputs (optional, novo per adr-059); derived-regenerated (regenerados como consequência indireta) → derivedArtifacts (optional). Cada path inspeccionável: existing-altered validável por filesystem; new-created/derived-regenerated rastreável a decision item específico. tq-adr-03 fail se path fictício. ADRs pré-adr-059 grandfathered (sem plannedOutputs); novas ADRs SHOULD usar plannedOutputs."
			}, {
				...
				action: "Verificar tq-adr-03 (paths em affectedArtifacts, plannedOutputs e derivedArtifacts são reais)"
				detail: "Para cada path: ls do filesystem (existing-altered → affectedArtifacts) OU referência explícita a decision item específico criando o path (new-created → plannedOutputs per adr-059) OU referência a tooling regenerador (derived-regenerated → derivedArtifacts). Path especulativo ou aspiracional sem rastreabilidade falha. Disciplina 3-way agora schema-supported: classificação determina o field, não rationale narrative."
			}]
			sources: [
			...
			heuristics: [
				...
				"Disciplina 3-way schema-supported (per adr-059): agente classifica path em existing-altered → affectedArtifacts; new-created → plannedOutputs; derived-regenerated → derivedArtifacts. Schema enforce shape via cue vet; PG enforce semantic completude via tq-adrg-03. ADRs pré-adr-059 grandfathered.",
				"affectedArtifacts: cada path existing-altered validável por inspeção (filesystem). plannedOutputs: cada path new-created rastreável a decision item específico. derivedArtifacts: cada path derived-regenerated.",
			...
			doneCriteria: "consequences com listas positiva (≥1 item) e negativa (≥1 item) específicas e observáveis; rationale articula coerência com princípios + trade-offs (sem duplicar decision); affectedArtifacts (existing-altered) + plannedOutputs (new-created, optional) + derivedArtifacts (derived-regenerated, optional) com paths reais sob disciplina 3-way schema-supported (per adr-059); principlesApplied com IDs verificados em design-principles.cue; tq-adr-02 e tq-adr-03 verificados."
	...
	finalValidation: {
		steps: [
			...
			"Verificar tq-adr-03 / tq-adrg-03 (hardened fail): para cada ADR, paths em affectedArtifacts (existing-altered, required), plannedOutputs (new-created, optional per adr-059), derivedArtifacts (derived-regenerated, optional) existem no filesystem OU são outputs diretos/derivados rastreáveis a decision items específicos. Disciplina 3-way schema-supported: existing-altered → affectedArtifacts; new-created → plannedOutputs; derived-regenerated → derivedArtifacts. ADRs pré-adr-059 grandfathered (sem plannedOutputs; affectedArtifacts conflate existing-altered + new-created).",
