...
agentSpecGuide: artifact_schemas.#ProductionGuide & {
	...
	_qualityCriteria: {
		criteria: [{
			...
			id:          "tq-agg-11"
			description: "Guide enforça cobertura das 6 famílias + exclusão consciente legítima (adr-175)"
			test:        "Process da section scope-and-action-catalog inclui passo de decidir cobertura vs exclusão para o catálogo operável (6 famílias incl. domainServices); heuristics declaram o critério de exclusão legítima do adr-175 (estrutural OU doutrinária, nunca conveniência) e a preferência por classe quando ≥3 ids caem sob a mesma regra; finalValidation inclui passo de rodar o runner e confirmar zero itens do BC no sc-ag-02. Verificado por inspeção do guide."
			severity:    "warn"
			rationale:   "O gate sc-ag-02 (adr-175) acusa mecanicamente id nem coberto nem excluído, mas a LEGITIMIDADE da exclusão é interpretativa (P10 — leitura guiada pelo critério, não gate LLM). O guide força a decisão consciente durante a autoria; sem isso, a próxima autoria de agent-spec recria o drift que as fatias WI-151/152/153 revelaram."
		}]
		rationale: "11 critérios cobrem disciplinas core para autoria de agent-spec: integridade referencial actions↔domain-model (tq-agg-01), invariant→constraint coverage (tq-agg-02), escalation coherence (tq-agg-03), observability completude (tq-agg-04), enforcementLevel per constraint (tq-agg-05), derivedFromInvariant explicit ref (tq-agg-06), action impact classification (tq-agg-07), per-action escalation override (tq-agg-08), decision-vs-execution separation (tq-agg-09), canonical test domínio-é-centro (tq-agg-10), cobertura 6 famílias + exclusão legítima (tq-agg-11). Critérios 01-04 derivam dos tq-ag-XX do schema; 05-10 derivam de revisão estrutural pós-PG-A (sessão 2026-05-01) — todos warn por enquanto, heuristic-level até schema absorver; 11 deriva do gate agente↔modelo (adr-175, elo duplo — critério assegura que o guide contém a disciplina, os passos em process/finalValidation são o operativo). Cobertura completa dos 13+ tq-ag-XX do schema vive em finalValidation.steps."
	}
	...
	sections: {
		"scope-and-action-catalog": {
			...
			process: [{
				...
				action: "Popular operationalScope com refs ao domain-model (6 famílias per adr-175)"
				detail: "operationalScope.aggregates obrigatório (≥1 ref a aggregates[].code do domain-model do BC). commands/events/invariants/projections/domainServices opcionais conforme escopo do agente — domainServices (svc-) é a 6ª família per adr-175: lógica cross-aggregate que o agente INVOCA (categoria-ação). Refs validadas por sc-ag-01 (reject) — building block inexistente é responsabilidade fantasma."
			}, {
				action: "Decidir cobertura vs exclusão para o catálogo operável (sc-ag-02, adr-175)"
				detail: "O gate sc-ag-02 exige que TODO id das 6 famílias operáveis do domain-model do BC esteja coberto (operationalScope/actions[].domainModelRefs) OU excluído em scopeExclusions — nem coberto nem excluído é drift agente↔modelo. Para cada building block fora da responsabilidade do agente, declarar exclusão: por id ({ref, rationale}) para casos individuais; por classe ({class, rationale, refs}) quando vários ids caem sob a mesma regra (ex.: policy-issued-commands, engine-enforced-invariants, external-actor-commands). Exclusão só é legítima pelo critério do adr-175: classe estruturalmente identificável (ex.: derivável de policies[].issuesCommand) OU doutrinariamente fechada com rationale citando a marcação em prosa do building block. 'Dava trabalho cobrir' não satisfaz. Ids excluídos são validados como existentes no domain-model (sc-ag-01 — dangling é violação)."
			}, {
				action: "Catalog actions[] com matrix category × autonomyLevel × inputTrustLevel"
				detail: "Cada action declara: code (act- prefix); category (query/observation/mutation/validation/generation/escalation; observation added per ADR-089); autonomyLevel (default 'propose-and-wait' para mutations em onboarding — Phase 0 100% supervisão); inputTrustLevel (obrigatório se action processa input externo per tq-ag-11 warn — trusted-internal/external-structured/external-untrusted-freeform); domainModelRefs ⊆ operationalScope (tq-ag-02 fail); preconditions/postconditions opcionais. Action count proporcional à complexidade do BC."
			}, {
			...
			heuristics: [
				...
				"domainModelRefs em actions[] que apontam para categorias declaradas em operationalScope (agg-, cmd-, evt-, inv-, prj-, svc-) DEVEM ser subset de operationalScope (least privilege per tq-ag-02 fail) — svc- ganhou família própria (domainServices) per adr-175. Refs com prefixos não representados diretamente em operationalScope (vo-, ent-, qry-, mod-, pol-) não exigem entrada literal: vo-/ent- são associados via aggregate parent; qry- via projection parent; mod- é agrupamento organizacional e pol- é automação determinística de runtime (P10) — nenhum dos dois é 'operado' pelo agente. Schema #DomainModelRef regex permite todos esses prefixos. Action operando fora do escopo é responsabilidade não-declarada.",
				"Cobertura ≠ exclusão ≠ omissão (adr-175): o par operationalScope+scopeExclusions particiona o catálogo operável do BC — o que o agente opera (coberto) e o que conscientemente NÃO opera (excluído com rationale). Omissão silenciosa é o que o sc-ag-02 acusa. Preferir exclusão por classe quando ≥3 ids caem sob a mesma regra (1 rationale auditável em vez de carimbos repetidos); a classe é o fundamento, refs[] é a extensão verificável que o runner resolve.",
		...
		"context-observability-validation": {
			...
			process: [{
				...
				detail: "Para cada category presente em actions[] (query/observation/mutation/validation/generation/escalation; observation added per ADR-089), declarar ≥1 signal com sig- prefix em observability.signals. 6 codes canônicos recorrentes: sig-mutation-executed, sig-validation-result, sig-escalation-triggered, sig-query-served, sig-supervision-requested, sig-constraint-violation. Domain-specific signals adicionais permitidos."
			}, {
	...
	finalValidation: {
		steps: [
			...
			"Verificar integridade referencial actions↔domain-model (tq-ag-01/02 + tq-agg-01 + sc-ag-01): cada ref em operationalScope (incl. domainServices per adr-175) E cada id em scopeExclusions (ref/refs[]) existe em domain-model do BC — exclusão dangling é violação; cada actions[].domainModelRefs com prefixos de operationalScope (agg-/cmd-/evt-/inv-/prj-/svc-) é subset de operationalScope (least privilege); refs com prefixos não representados diretamente em operationalScope (vo-/ent-/qry-/mod-/pol-) são associadas via parent (vo-/ent-→aggregate; qry-→projection) ou doutrina própria (mod- organizacional; pol- automação de runtime) sem entrada literal exigida.",
			"Verificar cobertura do catálogo operável (sc-ag-02, adr-175): todo id das 6 famílias (aggregates/commands/events/invariants/projections/domainServices) do domain-model do BC está coberto (operationalScope/actions) OU excluído (scopeExclusions por id/classe). Cada exclusão satisfaz o critério de legitimidade do adr-175 (classe estruturalmente identificável OU doutrinariamente fechada com rationale citando a marcação em prosa) — exclusão-carimbo ('dava trabalho cobrir') é o cenário (a) da falsificationCondition do adr-175 e falha review. Rodar o runner estrutural e confirmar zero itens do BC no anúncio do sc-ag-02.",
			...
			"Verificar observability completude (tq-ag-05 warn + tq-agg-04): signals[] cobre ≥1 por category presente em actions[] (query/observation/mutation/validation/generation/escalation; observation added per ADR-089). 6 codes canônicos como base; domain-specific adicionais permitidos.",
