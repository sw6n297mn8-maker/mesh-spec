...
agentSpecGuide: artifact_schemas.#ProductionGuide & {
	...
	sections: {
		"scope-and-action-catalog": {
			...
			heuristics: [
				...
				"domainModelRefs em actions[] que apontam para categorias declaradas em operationalScope (agg-, cmd-, evt-, inv-, prj-) DEVEM ser subset de operationalScope (least privilege per tq-ag-02 fail). Refs com prefixos não representados diretamente em operationalScope (vo-, ent-, qry-, mod-, svc-, pol-) não exigem entrada literal: vo-/ent- são associados via aggregate parent; qry- via projection parent; mod-/svc-/pol- seguem scope próprio quando declarados como building blocks top-level. Schema #DomainModelRef regex permite todos esses prefixos. Action operando fora do escopo é responsabilidade não-declarada.",
	...
	finalValidation: {
		steps: [
			...
			"Verificar integridade referencial actions↔domain-model (tq-ag-01/02 + tq-agg-01): cada ref em operationalScope existe em domain-model do BC; cada actions[].domainModelRefs com prefixos de operationalScope (agg-/cmd-/evt-/inv-/prj-) é subset de operationalScope (least privilege); refs com prefixos não representados diretamente em operationalScope (vo-/ent-/qry-/mod-/svc-/pol-) são associadas via parent (vo-/ent-→aggregate; qry-→projection) ou cobertas por scope próprio (mod-/svc-/pol- top-level) sem entrada literal exigida.",
