...
#DesignSystemConstitution: {
	...
	_qualityCriteria: #QualityCriteria & {
		criteria: [{
			...
			test:        "Cada token declara derivesFrom (enum fechado — shape via cue vet) apontando a camada/seção de onde deriva, e o changeRegime é justificado pelo texto citado em constraints sob o CRITÉRIO DE VÍNCULO da seção VII (tokenRegime, emenda 1.1): constitution-bound apenas quando não resta valor livre a calibrar — o token expressa relação, caráter ou proibição cuja alteração, por si, muda significado, identidade, distinção semântica ou personalidade; calibratable quando a camada fixa moldura e resta valor dentro dela, caso em que constraints declara a MOLDURA (piso, teto, relação, emprego, condição) e o valor vigente fica no runtime. Um valor não é constitution-bound por aparecer literalmente no texto promulgado — o teste é causal: qual lei superior deixaria de valer se ele mudasse. Token cujo constraints não permite decidir o regime por esse teste falha."
			severity:    "warn"
			rationale:   "O regime por token é o que torna a distinção manutenção-vs-emenda (VII) operável: commit no runtime dentro da moldura vs emenda via ADR. Critério textual em vez de causal promove craft a lei por acidente de redação — a falha que adr-195 corrigiu."
		}]
