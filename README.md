...
| `architecture/deferred-decisions` | Deferimentos conscientes governados: decisões explícitas de não resolver agora, com trade-off articulado e condição codificada de revisita (per adr-062). | Nome no formato def-NNN-slug.cue com numeração contínua e incremental.<br>Conformam com architecture/artifact-schemas/deferred-decision.cue.<br>Status inicial open; lifecycle avaliado por scripts/ci/evaluate-deferred-triggers.sh (runner não muta arquivos). |
| `architecture/design-system` | Constituição do Design System Mesh: a lei da expressão — identidade, camadas, contrato de tokens e regime de mudança (adr-194). | Instância única composta por merge de structs CUE: constitution.cue + canonical-cases.cue + token-contract.cue compõem UM designSystemConstitution.<br>Camadas congeladas: emenda exige ADR apontando o elo superior mudado (cláusula IX); token calibratable recalibra por commit no frontend-runtime dentro da moldura; token constitution-bound exige emenda.<br>Casos canônicos novos são candidatos — nunca entram sem decisão do founder.<br>Preservação é lei: o texto promulgado pelo founder não é resumido, parafraseado nem 'melhorado'. |
...
| `contexts` | Layer 2 da espec: bounded contexts autocontidos, cada um um pacote completo. | Um subdiretório por BC, nome lowercase de 3 letras (ex.: cmt, ssc, bkr).<br>Cada BC contém canvas, invariants, commands, events, workflows e demais artefatos declarados em governance/bounded-context-completeness.cue. |
| `contexts/bdg` | Bounded Context Budget & Approval: verifica cobertura orçamentária de compromissos formalizados em CMT e emite o gate canônico de aprovação que habilita a progressão do commitment lifecycle. | canvas.cue é a porta de entrada do BC; leitura obrigatória antes de alterar artefato interno.<br>Eventos deste BC são consumidos por contratos declarados em strategic/context-map.cue. |
| `contexts/bdg/agents` | Specs e governance envelopes dos agentes do BC Budget & Approval. | Um par de arquivos por agente: {agent-slug}.cue e {agent-slug}.governance.cue.<br>Specs conformam com architecture/artifact-schemas/agent-spec.cue.<br>Governance envelopes conformam com architecture/artifact-schemas/agent-governance.cue. |
| `contexts/bkr` | Bounded Context Banking Rails & Settlement: liquidação física via rails bancários (Pix/SPI, TED/STR, boleto, SWIFT) sob autorização upstream — boundary entre a Mesh e o sistema regulado pelo Bacen. | canvas.cue é a porta de entrada do BC; leitura obrigatória antes de alterar artefato interno.<br>Eventos deste BC são consumidos por contratos declarados em strategic/context-map.cue. |
| `contexts/bkr/agents` | Specs e governance envelopes dos agentes do BC Banking Rails & Settlement. | Um par de arquivos por agente: {agent-slug}.cue e {agent-slug}.governance.cue.<br>Specs conformam com architecture/artifact-schemas/agent-spec.cue.<br>Governance envelopes conformam com architecture/artifact-schemas/agent-governance.cue. |
...
| `contexts/dlv` | Bounded Context Delivery & Verification: verifica execução operacional contra critérios versionados acordados em CMT e decide deterministicamente a suficiência de evidência para progressão econômica. | canvas.cue é a porta de entrada do BC; leitura obrigatória antes de alterar artefato interno.<br>Eventos deste BC são consumidos por contratos declarados em strategic/context-map.cue. |
| `contexts/dlv/agents` | Specs e governance envelopes dos agentes do BC Delivery & Verification. | Um par de arquivos por agente: {agent-slug}.cue e {agent-slug}.governance.cue.<br>Specs conformam com architecture/artifact-schemas/agent-spec.cue.<br>Governance envelopes conformam com architecture/artifact-schemas/agent-governance.cue. |
| `contexts/fce/agents` | Specs e governance envelopes dos agentes do BC Financial Commitment Execution. | Um par de arquivos por agente: {agent-slug}.cue e {agent-slug}.governance.cue.<br>Specs conformam com architecture/artifact-schemas/agent-spec.cue.<br>Governance envelopes conformam com architecture/artifact-schemas/agent-governance.cue. |
...
| `contexts/idc/agents` | Specs e governance envelopes dos agentes do BC Identity & Data Governance. | Um par de arquivos por agente: {agent-slug}.cue e {agent-slug}.governance.cue.<br>Specs conformam com architecture/artifact-schemas/agent-spec.cue.<br>Governance envelopes conformam com architecture/artifact-schemas/agent-governance.cue. |
| `contexts/inv` | Bounded Context Invoicing: materializa a obrigação de faturamento (InvoiceIssued) e o direito creditório a partir de DeliveryVerified, sob regime fiscal regulado determinístico. | canvas.cue é a porta de entrada do BC; leitura obrigatória antes de alterar artefato interno.<br>Eventos deste BC são consumidos por contratos declarados em strategic/context-map.cue. |
| `contexts/inv/agents` | Specs e governance envelopes dos agentes do BC Invoicing. | Um par de arquivos por agente: {agent-slug}.cue e {agent-slug}.governance.cue.<br>Specs conformam com architecture/artifact-schemas/agent-spec.cue.<br>Governance envelopes conformam com architecture/artifact-schemas/agent-governance.cue. |
...
| `contexts/p2p` | Bounded Context Procure-to-Pay: emite Purchase Orders sob autoridade de sourcing pré-existente e faz hand-off da demanda formalizada para CMT — gateway entre sourcing (SSC) e compromisso. | canvas.cue é a porta de entrada do BC; leitura obrigatória antes de alterar artefato interno.<br>Eventos deste BC são consumidos por contratos declarados em strategic/context-map.cue. |
| `contexts/p2p/agents` | Specs e governance envelopes dos agentes do BC Procure-to-Pay. | Um par de arquivos por agente: {agent-slug}.cue e {agent-slug}.governance.cue.<br>Specs conformam com architecture/artifact-schemas/agent-spec.cue.<br>Governance envelopes conformam com architecture/artifact-schemas/agent-governance.cue. |
| `contexts/rew` | Bounded Context Risk Engine & Risk Observability: avaliação de risco e elegibilidade como camada derivada da operação verificada da rede, consolidando sinais para BCs consumidores (SCF, CMT e outros). | canvas.cue é a porta de entrada do BC; leitura obrigatória antes de alterar artefato interno.<br>Eventos deste BC são consumidos por contratos declarados em strategic/context-map.cue. |
| `contexts/rew/agents` | Specs e governance envelopes dos agentes do BC Risk Engine & Risk Observability. | Um par de arquivos por agente: {agent-slug}.cue e {agent-slug}.governance.cue.<br>Specs conformam com architecture/artifact-schemas/agent-spec.cue.<br>Governance envelopes conformam com architecture/artifact-schemas/agent-governance.cue. |
| `contexts/ssc` | Bounded Context Strategic Sourcing & Category: decide qual fornecedor atende qual categoria via regras determinísticas sobre sinais estruturados (NPM, RFQ, respostas); início do macrofluxo Mesh. | canvas.cue é a porta de entrada do BC; leitura obrigatória antes de alterar artefato interno.<br>Eventos deste BC são consumidos por contratos declarados em strategic/context-map.cue. |
| `contexts/ssc/agents` | Specs e governance envelopes dos agentes do BC Strategic Sourcing & Category. | Um par de arquivos por agente: {agent-slug}.cue e {agent-slug}.governance.cue.<br>Specs conformam com architecture/artifact-schemas/agent-spec.cue.<br>Governance envelopes conformam com architecture/artifact-schemas/agent-governance.cue. |
...
| `strategic/economic-model` | Layer -1 (Economic Reality, ri-*) + Layer 1 (Economic Mechanisms, mech-*): foundation econômica (adr-082/083). Layer -1 declara realidades adversariais; Layer 1, mecanismos que reduzem exploitability. | Singleton instances: mesh-economic-assumptions.cue (Layer -1) + mesh-economic-mechanisms.cue (Layer 1).<br>Prefix discipline Layer -1: ri-* reality invariants (DISTINCT de inv-* domain invariants); cap-adv-* capabilities; imp-* implications.<br>Prefix discipline Layer 1: mech-* economic mechanisms (DISTINCT de mech-* subdomain mechanisms via #EconomicMechanismRef alias); rr-* residual risks.<br>Reality invariants são propriedades do mundo NÃO tensionáveis (sistema sobrevive apesar de). Economic mechanisms reduzem exploitability — não eliminate, não solve.<br>Honesty enforcement: cada mechanism MUST declare failure surface (falsePositiveRisks OR underspecifications OR residualRisks) per tq-emm-03 — hidden risk inválido por discipline. |
...
Tree DERIVADA dos _meta.cue por diretório (adr-115): entries := treeEntries, gerado por scripts/ci/generate-repo-tree.py a partir do scan de scope.validated. Cobre os 56 diretórios governados; .claude/ e scripts/ ficam fora por estarem em scope.excluded (infra/tooling, não spec). purpose/conventions/rationale co-localizados no _meta.cue de cada diretório (P0).
...
O `assertion-schema.cue` (em `architecture/shared-schemas/`) define a gramática formal: `#Assertion` (subject, variables, predicate, rationale); `#Variable` (name, source — enum fechado `event-log|command|aggregate-state|projection|contract` —, filter opcional); `#Term` (`VarRef` | `Literal`; aritmética multi-termo deferida a def-054); `#Predicate` como folha (uma relação sobre `#Term`: binária `eq/neq/lt/lte/gt/gte/in/subset-of` exige `right`, unária `exists`/`not-exists` não) ou composição booleana (`and`/`or`/`implies`/`not`). Todo `invariants.cue`, `policies.cue` e `state-models.cue` importa esse schema. CUE é o container das assertions — não a engine de avaliação. Um gerador de testes no CI do mesh-runtime consome as assertions e produz property-based tests — a `#Assertion` é artifact-class de primeira ordem do pipeline de codegen (adr-140 item 6). O padrão é o mesmo de CUE → `.proto`: CUE define estrutura, codegen produz artefato executável.
