...
p2pPrimaryAgent: artifact_schemas.#AgentSpec & {
	...
	actions: [{
		...
		description: "Validar estruturalmente payload de cmd-emit-purchase-order antes de aggregate creation: presença de campos obrigatórios (categoryRef + supplier + scope com items ≥1 — cada vo-purchase-item com itemId/description/quantity/unit — + deadline/location per adr-198 + amount Money + claimedAuthorityRef + requestedBy), formato de Money (currency ISO 4217 + amount ≥ 0), consistência de scope (quantity positiva por item + deadline futuro), supplier formato vo-supplier-ref válido. Pré-condição de act-emit-purchase-order. Impact: read-only (validação técnica sobre payload de demanda)."
		category:        "validation"
		autonomyLevel:   "execute-and-log"
		inputTrustLevel: "trusted-internal"
		domainModelRefs: [
		...
		description: "Processar a decisão de aprovação do gestor por Alçada sobre requisição triada — o PORTÃO DUPLO pré-pedido (adr-174 + adr-177): (braço 1, cobertura) roda a interação sync com o Gate de Cobertura do bdg (cmd-approve-budget keyed por requisitionRef, fase 1 do two-phase) e verifica reserva confirmada via QueryBudgetApprovalStatus (status=reserved — inv-approval-requires-coverage-reservation, padrão adr-055); (braço 2, procedência de preço, POR LINHA per adr-198) resolve, para cada vo-approval-line, o vo-item-award da decisão referenciada por sourcingDecisionRef + sourcingItemId via QueryQuotationMap (sync, call-site fora do grafo per adr-120) e verifica o gate determinístico inv-approval-amount-matches-winning-quotation (decisão existe e concluiu; item-award com outcome=awarded e linha vencedora resolvível — linha contra item no-quotation/withheld escala; currency match por linha; unitPrice × quantity == lineAmount por linha — quantity FIRME da linha, NUNCA a estimada do escopo; Σ lineAmount == amount); com os DOIS braços verdes E decisão approve do gestor, processa cmd-approve-purchase (triaged→approved, evt-purchase-approved carregando sourcingDecisionRef + lines — o elo no nível do item existe no disco, def-088 resolvido pelo adr-198); decisão reject processa a transição triaged→rejected (evt-purchase-approval-rejected); falha de QUALQUER braço NÃO transiciona (requisição permanece triaged; braço bdg: escalada supervisionada do bdg; braço ssc: divergência de linha ou de soma escala aqui; no one-shot o vencedor por item é único por construção — ambiguous-case fica restrito a decisão preferred/strategic sem itens). A DECISÃO é humana (gestor por Alçada — sh-09 gestor-aprovador no stakeholder-map per WI-157/def-076 resolved; a operacionalização do papel na borda é o ADR de identidade do WI-158); o agente executa as interações e o processamento. Phase 0 propose-and-wait. Impact: state-change + cross-bc (interações sync com bdg e ssc per adr-055)."
		category:        "mutation"
		autonomyLevel:   "propose-and-wait"
		inputTrustLevel: "trusted-internal"
		domainModelRefs: [
	...
	observability: {
		signals: [{
			...
			description:    "Sinal P2P-specific emitido após PurchaseOrderEmitted publicado. Captura: purchaseOrderId + authorityRef + authorityType + supplier + categoryRef + amount + scope.items (itens per adr-198) + emittedAt + emittedBy + requestedBy + decisionRationaleLink (referência ao SSC sourcingDecisionId vinculado para reconstrução do gate independente do agent log para auditoria contínua cap-04 + insumo NIM futuro paralelo a oq-ssc-2)."
			coversCategory: "mutation"
