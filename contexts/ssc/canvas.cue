...
canvas: artifact_schemas.#Canvas & {
	...
	communication: {
		inbound: [{
			...
			description: "Retorna o mapa de cotações por rfqId (com filtro por categoria) — a MATRIZ ITEM × FORNECEDOR (adr-198): uma linha por item do escopo, uma coluna por fornecedor, equalização TCO derivada POR LINHA, célula vazia onde o fornecedor não cotou (proposta parcial), vencedor POR LINHA quando a decisão existe (itemAwards, com no-quotation/withheld visíveis), status da janela. Consumido por comprador (a comparação que suporta a escolha — passo do mapa da ds-buyer-procurement-journey, WI-152), supervisor e auditoria — consumidores INTRA-organização — e por P2P no 2º braço do portão de aprovação (adr-177/adr-198): resolução da LINHA vencedora por sourcingDecisionRef + sourcingItemId para o gate de procedência por linha (inv-approval-amount-matches-winning-quotation; relação ssc-to-p2p hybrid no context-map). NUNCA exposto a fornecedores (confidencialidade competitiva: os events de cotação são internal e não propagam cross-BC; o consumo P2P é sistema-a-sistema no gate, nunca superfície de fornecedor)."
		}]
		outbound: [{
