...
sscPrimaryAgent: artifact_schemas.#AgentSpec & {
	...
	actions: [{
		...
		description: "Gerar vo-decision-rationale estruturado capturando criteria aplicados (per fitness rule snapshot), weights vigentes da categoria, evaluatedSuppliers POR LINHA (todos cotantes válidos com itemId + scoresPerCriterion + finalRank na linha disputada — adr-198), tradeoffs articulados (justificativa de escolha vs alternativa específica em determinado critério). Quotations com status=withdrawn NÃO entram em evaluatedSuppliers; linha sem cotante válido não gera entradas (o vo-item-award registra no-quotation). Anexado ao audit trail (via decisionRationaleRef/hash) e ao payload do decision event (objeto completo). Sustenta inv-decision-rationale-required + cap-04 auditoria contínua + moat de inteligência (consumo NIM futuro pendente oq-ssc-2). Impact: read-only (geração de descritor; sem state change)."
		category:        "generation"
		autonomyLevel:   "execute-and-log"
		inputTrustLevel: "trusted-internal"
		domainModelRefs: [
	...
	constraints: [{
		...
		description:  "act-evaluate-and-conclude-rfq NUNCA emite decisão sem decisionRationale completo: criteria aplicados (per fitness rule snapshot), weights vigentes da categoria, evaluatedSuppliers POR LINHA (todos os cotantes válidos com itemId + scoresPerCriterion + finalRank — adr-198), tradeoffs articulados (justificativa de escolha vs alternativa específica em determinado critério). decisionRationale vazio ou parcial bloqueia emissão. Quotations com status=withdrawn NÃO entram em evaluatedSuppliers; linha sem cotante válido registra no-quotation no vo-item-award correspondente."
