...
canvas: artifact_schemas.#Canvas & {
	...
	openQuestions: [{
		...
		impact:   "PARCIALMENTE materializada: api.yaml tem a LEITURA — estado por id (WI-143) + fila de escalados (WI-144) — e o primeiro COMMAND supervisionado: resolve-guard-escalation (WI-146, sob def-024 — supervisorId nominal não-verificado na borda até ADR de auth); async-api.yaml materializado (WI-147 — o gatilho real do item c, re-datado no housekeeping 2026-07-03, disparou: fatia browser-live do frontend-runtime aberta) — 2 channels publish-only (payment-settled, payment-obligation-defaulted) no molde CMT/DLV/INV sob def-023 (sem bindings/servers); sc-cv-03 do FCE SATISFEITO (flags true/true, ambos os specs no disco). Segue em aberto, RE-DATADO com gatilho real (housekeeping 2026-07-03, decisão do founder): (b) commands do caminho autônomo — SEM consumidor materializado e contra o inbound event-driven declarado (o organismo os dirige por eventos); re-avaliar SE um consumidor HTTP real aparecer, não autorar por prazo."
		deadline: "2026-09-30"
