...
// tem 6 (NAO inclui evt-payment-obligation-defaulted: evento de CATALOGO sem
// emissor na fatia -- o fluxo de default esta fora, T2 do domain-model).
//
// adr-155 (override humano do PrePaymentGuard): +1 command
// (cmd-resolve-guard-escalation), +3 events (guard-escalated/overridden/
// override-refused), +2 invariants (override-requires-attribution + o piso
// inv-breach-bypasses-escalation). Sync verbatim com agg-payment mantido.
...
aggregateManifest: artifact_schemas.#AggregateManifest & {
	...
	// 5 commands verbatim de agg-payment.handlesCommands.
	commandsAccepted: [
	...
	// 6 events verbatim de agg-payment.emitsEvents (Defaulted fora — catalogo).
	eventsEmitted: [
	...
	// 7 invariants verbatim de agg-payment.protectsInvariants (5 do subconjunto
	// declarado das 11 do Payment no canvas + 2 do adr-155: atribuicao do
	// override e o piso inoverridavel).
	invariants: [
	...
	generatedArtifacts: [{
		...
		description: "Aggregate base/skeleton do Payment derivado deste manifest (adr-141 item 5): estados do lifecycle (guarded, escalated, authorized, dispatched, settled, refused — #PaymentState reusado dos schemas, disjuncao fechada), handlers dos 5 commands, emissao dos 6 events declarados, guards das 7 invariants."
