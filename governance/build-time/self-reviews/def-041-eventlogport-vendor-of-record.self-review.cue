...
def041: build_time.#SelfReviewReport & {
	...
	roundDetails: [{
		...
		infoCount: 0
		summary: """
			Self-review do def-041 (vendor-of-record do EventLogPort — SoT dos fatos, categoria event
			store append-only — deferido). Avaliado contra tq-def-01..04 + universalCriteria.
			tq-def-01 (trade-off concreto): deferralRationale articula custo evitado (especular um event
			...
			(low, fora do caminho crítico) e o EventLogPort é consumido por todos os BCs (cross-cutting);
			não é o par suspeito low+repo-wide; mesmo par de def-040. Pass.
			uq (Mesh): EventLogPort, três SoTs, reference adapter, harness WI-137 — específico e agnóstico
	...
	summary: """
		def-041 defere a seleção do vendor de event store que implementa o EventLogPort (SoT dos fatos)
