...
productionGuideMetaInstance: build_time.#SelfReviewReport & {
	...
	singleRoundRationale: """
		...
		formal pós-commit (refeito após commit 12 revertido por findings
		de vp-self-review-report — enum #ArtifactType estendida na mesma
		operação para incluir "production-guide"). Critério de
		estabilização em 1 round: conteúdo substantivo herdado de
	...
	roundDetails: [{
		...
		summary: """
			...
			é risco crítico); rationale de _schema.location explica WHY
			(fecha meta-recursão localmente em mesh). uq-02: ancoragem
			mesh via tq-as-05 cross-repo e referência à promoção FP-02
			upstream tekton. uq-03: target "#ProductionGuide" existe no
			schema mesh-local; sources cross-repo para tekton-spec/portfolio
			(business-case.base, repo-bootstrap-plan, claude-config)
			verificados existentes em /home/user/tekton-spec/portfolio/
			production-guides/. uq-04: zero contradição com design
			principles; P0 honrado (guide é localização canônica única).
			uq-05: limitações declaradas no rationale do _qualityCriteria
			(severities hardened deliberadamente). uq-06: terminologia
			consistente. uq-07: zero placeholder pós-commit 3/3. uq-08:
			shape #ProductionGuide satisfeito (cue vet limpo recursive).
			tq-pg-01: workOrder ["target-and-prerequisites",
			"sections-and-workorder", "validation-and-meta"] ===
			keys(sections), permutação exata sem duplicatas/omissões.
			tq-pg-02: todas as 3 sections.target = "#ProductionGuide"
			(válido no schema adotado). tq-pg-03: doneCriteria avaliáveis
			em todas as sections. tq-pg-04: prerequisites.gapPolicy com
			800+ runes E declara "NÃO crie", "NÃO invente", "NÃO infira",
			"NÃO copie". tq-pg-05: finalValidation.steps[-1] = "Submeter
			ao founder para aprovação antes de commit." tq-pg-06:
			process[].action começam com verbos imperativos concretos.
			Zero findings em qualquer severidade.
			"""
	}]
