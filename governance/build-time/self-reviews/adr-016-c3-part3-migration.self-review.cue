...
adr016C3Part3Migration: build_time.#SelfReviewReport & {
	reportId: "srr-adr-016-c3-part3-migration"
	...
	roundDetails: [{
		...
		warnCount: 0
		infoCount: 0
		summary:   "C3 Part 3 migration per adr-059: 1 path reclassificado de affectedArtifacts (existing-altered) para plannedOutputs (new-created): scripts/ci/check-readme-coevolution.sh. 1 path permanecem em affectedArtifacts (governance/repo-structure.cue). Verified via git archaeology (commit hash + timestamp comparison vs ADR commit). Editorial migration."
	}]
