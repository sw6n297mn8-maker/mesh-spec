...
// SRR do def-044 — re-deferimento pós-triagem (adr-162): trigger file-exists esgotado
...
def044: build_time.#SelfReviewReport & {
	...
	roundDetails: [{
		...
		infoCount: 0
		summary: """
			Re-review isolated-subagent (batch das 9 re-adiações, triagem adr-162) do def-044 após
			...
			o evento real (1º adapter persistente não-in-memory do DeliveryPort em construção), que vive
			no mesh-runtime (convenção platform/adapters/delivery-<vendor>/) sem sensor honesto no
	...
	summary: """
		def-044 RE-ADIADO na triagem do backlog disparado (adr-162): o trigger file-exists esgotado
		(proxy-prematuro em validate-codegen.sh) deu lugar a manual-review (1º adapter persistente do
		DeliveryPort — evento de runtime no mesh-runtime sem sensor honesto no mesh-spec) + temporal
	...
	singleRoundRationale: """
		...
		DeliveryPort) e o temporal 180d é o backstop gateável (adr-162). Shape, reason>=40, temporal
