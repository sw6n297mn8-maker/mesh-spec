...
def015TaskOutputTemporalityMetadata: build_time.#SelfReviewReport & {
	...
	roundDetails: [{
		...
		summary: """
			...
			Trigger calibrado pós-correção (2026-05-08): kind 'recurrence', pattern 'OUTPUTS STATUS [(]structured[)]:', scope file-content, threshold 5. Header line distintivo + character class [(] / [)] força match literal de parens (ERE interpreta () nu como group syntax — descoberto em teste). Threshold 5 absorve 3 self-references inevitáveis (def-015 + srr-def-015 + wi-070) + honra intent founder original '≥ 2 novas WIs emergent'.

			Pattern self-match check (CORRIGIDO em commit subsequent ao initial def-015 d97a443): SRR original alegava incorretamente que 'runner não confunde — pattern match em path scope diferente'. TESTE pós-write revelou bug duplo: (Bug A) regex parens não-escapadas; (Bug B) #Trigger schema scope=file-content NÃO suporta path-filter — git grep busca todo repo. Self-references contam como matches. Resolução documentada em def-015.triggerCalibrationRationale + threshold absorption pattern. Lesson learned: 'verificação por inspeção' insuficiente para regex-based triggers — exige test execution real (evaluate-deferred-triggers.sh) antes de commit final.
