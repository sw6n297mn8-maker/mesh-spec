...
adr074: build_time.#SelfReviewReport & {
	...
	roundDetails: [{
		...
		warnCount: 0
		infoCount: 0
		summary:   "ADR estende authoring-policy.cue rollout de 1 para 6 types: production-guide (existing) + canvas + glossary + domain-model + agent-spec + agent-governance (5 novos types de WI-048 BC bootstrap). promptTemplate generalizado: substitui hardcoded 'production-guide' por placeholder {artifactType}; substitui Section 1/2/3 references do META-PG por reference genérica a workOrder do production-guide alvo. outputContract tweaked: 'Draft #ProductionGuide' → 'Draft de instância conformante ao schema target'. Authorization-not-guarantee framing explícito: extensão do rollout AUTORIZA tentativa via subagent-drafted, NÃO garante sucesso; fallback to manual preserved per fallbackPolicy. Order de execução em WI-048: canvas → glossary → domain-model → agent-spec → agent-governance (dependency semântica). Revisit conditions explícitas: (a) 2+ fallbacks → revisar rollout, (b) canvas falha por complexidade → fora do rollout, (c) agent-spec/governance falham por context-heavy deps → restringir, (d) todos succedem → expand para próximos types. 4 alternativas explicitamente rejeitadas (status quo, canvas only B0, B1 escolhida, over-expansion). principlesApplied: P10, P12. decisionClass=structural, reversibility=high, blastRadius=cross-cutting. Cascade ordering pre-validated (5 PGs existem em main). Generalização at n=0 evidence per non-PG type aceita per founder guidance + WI-048 cohesion + per-dispatch fallback isolation. cue vet ./... EXIT=0; CI self-review-check local PASSED. AMENDMENT (post-commit ef5195f): claim original 'Cascade ordering pre-validated (5 PGs existem em main)' era factualmente incorreto. Inventário real pre-adr-074: 4 PGs (glossary, domain-model, agent-spec, agent-governance); canvas PG NÃO existia. Gap detectado em canvas instance dispatch (subagent honestamente flagged — primeiro priority item de reasoning report); resolved via canvas PG manual authoring em commit ef5195f. Decisão estrutural de adr-074 (extensão do rollout) permanece válida — apenas evidência de cascade ordering pre-validation foi corrigida factualmente. Edits do amendment em context bloco principal + consequences P4 + canvas rollout entry rationale em authoring-policy.cue. AMENDMENT 2 (post-commit 818c079): canvas removed from rollout. Revisit condition (b) triggered after 2 consecutive canvas-class timeouts (disp-003 PG canvas + disp-004 canvas instance). Diagnóstico: canvas-class workloads consistentemente excedem API timeout window por densidade estrutural inerente (~25 top-level fields + 167 sub-fields + 8 sections + cross-checks). Não tactical fallback — decisão estrutural sobre limite operacional de subagent-drafted. Canvas BDG materializado via manual authoring em commit 818c079 (canonical path until mechanism evolves). Pattern emergente capturado em Known gap: subagent-drafted viável para artefatos lineares + cross-checks limitados; falha para artefatos com múltiplas seções interdependentes + cross-checks intensivos. (P6) adicionado em consequences: materializa princípio mesh 'sistema robusto contra erro da própria IA' — falha é detection point, não failure mode. Edits do amendment 2 em context bloco principal + consequences P6 + Known gaps + canvas rollout entry removida em authoring-policy.cue."
	}]
