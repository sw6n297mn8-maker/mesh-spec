...
// SRR do def-042 — re-deferimento pós-triagem (adr-162): trigger file-exists esgotado
...
def042: build_time.#SelfReviewReport & {
	...
	roundDetails: [{
		...
		infoCount: 0
		summary: """
			Re-review isolated-subagent (batch das 9 re-adiações, triagem adr-162) do def-042 após
			...
			o evento real (1º adapter persistente não-in-memory do LedgerPort em construção), que vive
			no mesh-runtime (convenção platform/adapters/ledger-<vendor>/) sem sensor honesto no runner
			de mesh-spec — o vendor É a decisão; tq-def-03 satisfeito (temporal). date refrescada para
			2026-06-28 (idade 0 — não re-dispara) com a data ORIGINAL (2026-06-04) preservada no
	...
	summary: """
		def-042 RE-ADIADO na triagem do backlog disparado (adr-162): o trigger file-exists esgotado
		(proxy-prematuro em validate-codegen.sh) deu lugar a manual-review (1º adapter persistente do
		LedgerPort — evento de runtime no mesh-runtime sem sensor honesto no mesh-spec) + temporal 180d
	...
	singleRoundRationale: """
		...
		LedgerPort) e o temporal 180d é o backstop gateável (adr-162). Shape, reason>=40, temporal
