...
def042: build_time.#SelfReviewReport & {
	reportId: "srr-def-042-ledgerport-vendor-of-record"

	artifactPath:       "architecture/deferred-decisions/def-042-ledgerport-vendor-of-record.cue"
	...
	roundDetails: [{
		...
		infoCount: 0
		summary: """
			Self-review do def-042 (vendor-of-record do LedgerPort — SoT do valor, categoria ledger
			engine double-entry — deferido). Avaliado contra tq-def-01..04 + universalCriteria.
			tq-def-01 (trade-off concreto): deferralRationale articula custo evitado (especular um ledger
			engine antes de o harness WI-137 exercitar o Port = lock-in prematuro per P2/real-options) vs
			...
			(low, fora do caminho crítico) e o LedgerPort é consumido por todos os BCs (cross-cutting);
			não é o par suspeito low+repo-wide; mesmo par de def-040. Pass.
			uq (Mesh): LedgerPort, três SoTs, reference adapter, harness WI-137 — específico e agnóstico
	...
	summary: """
		def-042 defere a seleção do vendor de ledger que implementa o LedgerPort (SoT do valor) —
		categoria de runtime atrás do Port (P2), não contrato de domínio. Agnóstico (nenhum produto
		nomeado), forma def-040/049, deferimento JIT ancorado no harness de codegen-validation (WI-137).
		Estável em 1 round, 0 findings.
		"""
